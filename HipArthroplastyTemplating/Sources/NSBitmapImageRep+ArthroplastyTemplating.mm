...
struct ArthroplastyTemplatingIntegerPoint {
	NSInteger x, y;
    ArthroplastyTemplatingIntegerPoint(NSInteger x, NSInteger y) : x(x), y(y) {}
};
...
#define P(x,y) (y*rowBytes+x*pixelBytes)
	...
	std::stack<ArthroplastyTemplatingIntegerPoint> ps;
	// top and bottom borders
	for (NSInteger x = 0; x < width; ++x) {
		ps.push(ArthroplastyTemplatingIntegerPoint(x, 0));
		ps.push(ArthroplastyTemplatingIntegerPoint(x, height-1));
	}
	// left and right borders
	for (NSInteger y = 1; y < height-1; ++y) {
		ps.push(ArthroplastyTemplatingIntegerPoint(0, y));
		ps.push(ArthroplastyTemplatingIntegerPoint(width-1, y));
	}
	
	// flood from every point in stack
	while (!ps.empty()) {
        ArthroplastyTemplatingIntegerPoint p = ps.top();
		ps.pop();
		...
		if (data[P(p.x,p.y)+3] < alphaThreshold) { // if pixel is transparent, flood its neighbors
			data[P(p.x,p.y)+3] = 0;
			if (p.x > 0 && !visited[p.x-1][p.y]) ps.push(ArthroplastyTemplatingIntegerPoint(p.x-1, p.y));
			if (p.y > 0 && !visited[p.x][p.y-1]) ps.push(ArthroplastyTemplatingIntegerPoint(p.x, p.y-1));
			if (p.x < width-1 && !visited[p.x+1][p.y]) ps.push(ArthroplastyTemplatingIntegerPoint(p.x+1, p.y));
			if (p.y < height-1 && !visited[p.x][p.y+1]) ps.push(ArthroplastyTemplatingIntegerPoint(p.x, p.y+1));
		}
	}
